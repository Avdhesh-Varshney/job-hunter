...
def information(data, service):
  ...
  if category == 'HR':
    hr_names = sorted(data["hr_name"].unique())
...
def myStatus(data):
  ...
  filter_status = st.radio("Filter by Status", ['All', 'No Openings', 'Invitation Sent', 'In Talks'])
  if filter_status != 'All':
    data_display = data_display[data_display['status'] == status_color_map[filter_status]]

  unique_names = sorted(data_display['hr_name'].unique().tolist() + data_display['company_name'].astype(str).unique().tolist())
  ...
  items_per_page = st.slider("No. of Items per page", min_value=1, max_value=20, step=1, value=10)
  ...
  if total_pages > 1:
    ...
    st.dataframe(data_display.iloc[start_idx:end_idx])
